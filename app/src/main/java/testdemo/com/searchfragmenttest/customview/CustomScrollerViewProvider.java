package testdemo.com.searchfragmenttest.customview;
...
public class CustomScrollerViewProvider extends ScrollerViewProvider {
    ...
    public View provideHandleView(ViewGroup container) {
        ...
        int width = getContext().getResources().getDimensionPixelSize(R.dimen.custom_handle_width);
        int height = getContext().getResources().getDimensionPixelSize(R.dimen.custom_handle_height);
        handle.setLayoutParams(new ViewGroup.LayoutParams(width, height));
        Utils.setBackground(handle, drawRoundedRectangle(width, height, ContextCompat.getColor(getContext(), R.color.custom_handle_color)));
    ...
    public View provideBubbleView(ViewGroup container) {
        ...
        Utils.setBackground(bubble, drawRoundedRectangle(dimen, dimen, ContextCompat.getColor(getContext(), R.color.custom_bubble_color)));
        bubble.setVisibility(View.INVISIBLE);
        bubble.setGravity(Gravity.CENTER);
        bubble.setTextSize(32); // not dp
...
//        });
    ...
    protected ViewBehavior provideBubbleBehavior() {
        return new DefaultBubbleBehavior(new VisibilityAnimationManager.Builder(bubble).withHideDelay(1000).build());
    }
    ...
    private static ShapeDrawable drawRectangle (int width, int height, int color) {
    ...
    private static ShapeDrawable drawRoundedRectangle (int width, int height, int color) {
        float[] outerRadii = new float[] { 8, 8, 8, 8, 8, 8, 8, 8 };
        ShapeDrawable rect = new ShapeDrawable(new RoundRectShape(outerRadii, null, null));
        rect.setIntrinsicHeight(height);
        rect.setIntrinsicWidth(width);
        rect.getPaint().setColor(color);
        return rect;
